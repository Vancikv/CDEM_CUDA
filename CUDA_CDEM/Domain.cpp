...
Domain::Domain(int _nelems, int _nnodes, Node * _nodes, Element * _elements, float c_n, float c_s)
{
	nelems = _nelems;
	nnodes = _nnodes;
	nodes = _nodes;
...
void Domain::write_state_to_file(std::string filename, float time)
{
...
Eigen::Vector2f Domain::get_contact_force(int node_id)
{
	Eigen::Vector2f F(0., 0.);
	Eigen::Vector2f du_g;
	Eigen::Matrix2f T;
...
void Domain::solve(float t_load, float t_max, int maxiter)
{	
	...
	// Eigen matrices will be copied into arrays of floats.
	// Using the Eigen::Map function defaults in a column by column layout.
	float * u, *v, *a, *load, *supports, *K, *C, *Mi, *Kc, *n_vects;
	int * neighbors;
	...
	a = new float[vdim];
	load = new float[vdim];
	supports = new float[vdim];
	K = new float[nelems*stiffdim*stiffdim];
	C = new float[nelems*stiffdim];
	Mi = new float[nelems*stiffdim];
	Kc = new float[4];
	n_vects = new float[4 * nnodes];
	neighbors = new int[vdim];

	for (i = 0; i < nnodes; i++)
	{
		int of = i*nnodedofs;
		Eigen::Map<Eigen::VectorXf>(u + of, nodes[i].v_disp.rows(), nodes[i].v_disp.cols()) = nodes[i].v_disp;
		Eigen::Map<Eigen::VectorXf>(v + of, nodes[i].v_velo.rows(), nodes[i].v_velo.cols()) = nodes[i].v_velo;
		Eigen::Map<Eigen::VectorXf>(a + of, nodes[i].v_acce.rows(), nodes[i].v_acce.cols()) = nodes[i].v_acce;
		Eigen::Map<Eigen::VectorXf>(load + of, nodes[i].v_load.rows(), nodes[i].v_load.cols()) = nodes[i].v_load;
	...
	for (i = 0; i < nelems; i++)
	{
		...
		Eigen::Map<Eigen::MatrixXf>(K + of1, elements[i].K_loc.rows(), elements[i].K_loc.cols()) = elements[i].K_loc;
		Eigen::Map<Eigen::MatrixXf>(C + of2, elements[i].C_loc.diagonal().rows(), elements[i].C_loc.diagonal().cols()) = elements[i].C_loc.diagonal();
		Eigen::Map<Eigen::MatrixXf>(Mi + of2, elements[i].M_loc_inv.diagonal().rows(), elements[i].M_loc_inv.diagonal().cols()) = elements[i].M_loc_inv.diagonal();
	}

	Eigen::Map<Eigen::MatrixXf>(Kc, m_contact_stiffness.rows(), m_contact_stiffness.cols()) = m_contact_stiffness;
...
void Domain::solve(float t_load, float t_max, int maxiter, char* outfile, int output_frequency, int gridDim, int blockDim)
{
	float dt = t_max / maxiter;
	int i, j;
	float solve_begin_CPU = get_cpu_time();
	float solve_begin_wall = get_wall_time();
	for (i = 0; i < nelems; i++)
	{
		elements[i].set_matrices();
		elements[i].calc_normal_vectors();
	...
	float solve_end_CPU = get_cpu_time();
	float solve_end_wall = get_wall_time();
		...
		else if (lncount <= 1 + nnodes) // Node records
		{
			...
			while (std::getline(lss, entry, ' '))
			{
				if (entry == "ndofs")
				{
					...
					nd.v_load = Eigen::VectorXf::Zero(ndf);
					nd.v_disp = Eigen::VectorXf::Zero(ndf);
					nd.v_velo = Eigen::VectorXf::Zero(ndf);
					nd.v_acce = Eigen::VectorXf::Zero(ndf);
